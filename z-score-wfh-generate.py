#!/usr/bin/env python3
"""
Generates a DHIS2 program rule expression for uncapped WFH z-scores for OTP:

    python3 z-score-wfh-generate.py > z-score-wfh-formula-otp.txt
...
Program rule condition for the ASSIGN action using this expression:
    ...
    d2:floor(#{var_height_otp} * 2) >= 90 &&
    d2:floor(#{var_height_otp} * 2) <= 240

The condition is to prevent errors when the expression calls d2:zScoreWFH for the base value before optionally extending it if outside range of -3.5 to 3.5.
"""
...
HEIGHT_INDEX = f"d2:floor({HEIGHT} * 2)"
...
def parse_table(source: str, function_name: str) -> list[Row]:
    ...
    for line in lines:
        ...
        if match := ADD_ENTRY_RE.search(line):
            ...
            rows.append(Row(parse_float(match["height"]), *values))
...
def row_correction_expr(row: Row) -> str:
    ...
    return (
        f"({eq(HEIGHT_INDEX, n(row.height * 2))} * ("
...
def builtin_wfh(gender_literal: str) -> str:
    return f'd2:zScoreWFH({HEIGHT_INDEX} / 2,{WEIGHT},"{gender_literal}")'
