#!/usr/bin/env python3
"""Generates the WHO LMS based Nutrition Status program rule DHIS2 expression for OTP program.
...
Inputs:
#{var_weight_otp} (kg), #{var_height_otp} (cm), A{var_sex_otp} (male/female), #{var_muac_otp} (cm), #{var_oedema_otp} ("0" when absent).
...
def endpoint(rows: list[Row], parameters: list[dict[str, float]], power: str, side: str) -> str:
    ...
    return choose(f"{WEIGHT}<{lookup('sd3n')}", lower, central)
